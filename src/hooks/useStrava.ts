import { useState, useEffect, useCallback } from 'react';
import type { StravaActivity, StravaAuthState } from '../types';
import { stravaAuth } from '../services/stravaAuth';
import { stravaApi } from '../services/stravaApi';

interface UseStravaReturn {
  isConnected: boolean;
  isLoading: boolean;
  isSyncing: boolean;
  error: string | null;
  athleteName: string | null;
  activities: StravaActivity[];
  lastSyncTime: string | null;
  connect: () => void;
  disconnect: () => void;
  sync: (startDate: string, endDate: string) => Promise<void>;
}

export function useStrava(): UseStravaReturn {
  const [auth, setAuth] = useState<StravaAuthState | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isSyncing, setIsSyncing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [activities, setActivities] = useState<StravaActivity[]>([]);
  const [lastSyncTime, setLastSyncTime] = useState<string | null>(null);

  // Load stored auth and handle OAuth callback on mount
  useEffect(() => {
    const initialize = async () => {
      setIsLoading(true);

      // Check if returning from OAuth callback
      const callbackAuth = await stravaAuth.handleOAuthCallback();
      if (callbackAuth) {
        setAuth(callbackAuth);
        setIsLoading(false);
        return;
      }

      // Load stored auth
      const storedAuth = stravaAuth.getStoredAuth();
      setAuth(storedAuth);
      setIsLoading(false);

      // Load cached activities
      const cachedActivities = localStorage.getItem('strava_activities');
      if (cachedActivities) {
        try {
          setActivities(JSON.parse(cachedActivities));
        } catch {
          // Ignore parse errors
        }
      }

      const cachedSyncTime = localStorage.getItem('strava_last_sync');
      if (cachedSyncTime) {
        setLastSyncTime(cachedSyncTime);
      }
    };

    initialize();
  }, []);

  // Connect initiates OAuth flow
  const connect = useCallback(() => {
    stravaAuth.initiateOAuth();
  }, []);

  const disconnect = useCallback(() => {
    stravaAuth.clearAuth();
    setAuth(null);
    setActivities([]);
    setLastSyncTime(null);
    localStorage.removeItem('strava_activities');
    localStorage.removeItem('strava_last_sync');
  }, []);

  const sync = useCallback(async (startDate: string, endDate: string) => {
    if (!auth?.accessToken) {
      setError('Not connected to Strava');
      return;
    }

    setIsSyncing(true);
    setError(null);

    try {
      const fetchedActivities = await stravaApi.fetchActivitiesInDateRange(startDate, endDate);

      setActivities(fetchedActivities);
      localStorage.setItem('strava_activities', JSON.stringify(fetchedActivities));

      const syncTime = new Date().toISOString();
      setLastSyncTime(syncTime);
      localStorage.setItem('strava_last_sync', syncTime);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to sync activities';
      setError(errorMessage);

      // If we get an auth error, clear auth and prompt re-login
      if (errorMessage.includes('401') || errorMessage.includes('authorization')) {
        stravaAuth.clearAuth();
        setAuth(null);
      }
    } finally {
      setIsSyncing(false);
    }
  }, [auth?.accessToken]);

  return {
    isConnected: !!auth?.accessToken,
    isLoading,
    isSyncing,
    error,
    athleteName: auth?.athleteName ?? null,
    activities,
    lastSyncTime,
    connect,
    disconnect,
    sync,
  };
}
